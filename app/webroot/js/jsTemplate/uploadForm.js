...
var hasProp = !!jQuery.fn.prop;

// attr2 uses prop when it can but checks the return type for
// an expected string.  this accounts for the case where a form
// contains inputs with names like "action" or "method"; in those
// cases "prop" returns the element
jQuery.fn.attr2 = function() {
    if ( ! hasProp )
...
jQuery.fn.ajaxSubmit = function(options) {
    ...
    options = jQuery.extend(true, {
        ...
        type: method || jQuery.ajaxSettings.type,
    ...
    if (options.data) {
        ...
        qx = jQuery.param(options.data, traditional);
    }
    ...
    var q = jQuery.param(a, traditional);
    if (qx) {
        q = ( q ? (q + '&' + qx) : qx );
    }
    ...
    if (options.clearForm) {
        callbacks.push(function() { jQueryform.clearForm(options.includeHidden); });
    }
    ...
    if (options.error) {
        ...
        options.error = function(xhr, status, error) {
            ...
            oldError.apply(context, [xhr, status, error, jQueryform]);
        };
    }
     ...
     if (options.complete) {
        ...
        options.complete = function(xhr, status) {
            var context = options.context || this;
            oldComplete.apply(context, [xhr, status, jQueryform]);
        };
    }
    ...
    var fileInputs = jQuery('input[type=file]:enabled', this).filter(function() { return jQuery(this).val() !== ''; });
    ...
    if (options.iframe !== false && (options.iframe || shouldUseFrame)) {
        ...
        if (options.closeKeepAlive) {
            jQuery.get(options.closeKeepAlive, function() {
        ...
        jqxhr = jQuery.ajax(options);
    }

    jQueryform.removeData('jqxhr').data('jqxhr', jqxhr);
    ...
    function deepSerialize(extraData){
        var serialized = jQuery.param(extraData, options.traditional).split('&');
    ...
    function fileUploadIframe(a) {
        ...
        if (s.iframeTarget) {
            ...
            n = jQueryio.attr2('name');
            if (!n)
                 jQueryio.attr2('name', id);
            else
                id = n;
        }
        else {
        ...
        function doSubmit() {
            ...
            var t = jQueryform.attr2('target'), a = jQueryform.attr2('action');
                ...
                if (s.extraData) {
                    for (var n in s.extraData) {
                        if (s.extraData.hasOwnProperty(n)) {
                           // if using the jQuery.param format that allows for multiple values with the same name
                           if(jQuery.isPlainObject(s.extraData[n]) && s.extraData[n].hasOwnProperty('name') && s.extraData[n].hasOwnProperty('value')) {
                               extraInputs.push(
                               jQuery('<input type="hidden" name="'+s.extraData[n].name+'">').val(s.extraData[n].value)
                                   .appendTo(form)[0]);
                           } else {
                               extraInputs.push(
                               jQuery('<input type="hidden" name="'+n+'">').val(s.extraData[n])
                                   .appendTo(form)[0]);
                           }
                        }
                    }
        ...
        function cb(e) {
            ...
            if (status === 'success') {
                ...
                if (g)
                    jQuery.event.trigger("ajaxError", [xhr, s, errMsg]);
            }
...
jQuery.fn.ajaxForm = function(options) {
    options = options || {};
    options.delegation = options.delegation && jQuery.isFunction(jQuery.fn.on);
...
function doAjaxSubmit(e) {
    ...
    if (!e.isDefaultPrevented()) { // if event has been canceled, don't proceed
        e.preventDefault();
        jQuery(e.target).ajaxSubmit(options); // #365
    }
}
...
function captureSubmittingElement(e) {
    ...
    if (!(jQueryel.is("[type=submit],[type=image]"))) {
        // is this a child element of the submit el?  (ex: a span within a button)
        var t = jQueryel.closest('[type=submit]');
        if (t.length === 0) {
            return;
        }
        target = t[0];
 ...
 */
jQuery.fn.formToArray = function(semantic, elements) {
    var a = [];
    if (this.length === 0) {
        return a;
    }
    ...
    for(i=0, max=els.length; i < max; i++) {
        ...
        if (semantic && form.clk && el.type == "image") {
            ...
            if(form.clk == el) {
                a.push({name: n, value: jQuery(el).val(), type: el.type });
 ...
 *  </fieldset></form>
 *
 *  var v = jQuery('input[type=text]').fieldValue();
 *  // if no values are entered into the text inputs
 *  v == ['','']
 *  // if values entered into the text inputs are 'foo' and 'bar'
 *  v == ['foo','bar']
 *
 *  var v = jQuery('input[type=checkbox]').fieldValue();
 *  // if neither checkbox is checked
 *  v === undefined
 *  // if both checkboxes are checked
 *  v == ['B1', 'B2']
 *
 *  var v = jQuery('input[type=radio]').fieldValue();
 ...
 */
jQuery.fn.clearForm = function(includeHidden) {
    return this.each(function() {
        jQuery('input,select,textarea', this).clearFields(includeHidden);
    });
};
...
// expose debug var
jQuery.fn.ajaxSubmit.debug = false;
